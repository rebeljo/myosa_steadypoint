...
private:
  ...
  void updateMenu(){
    int y=joy.y();
    if(y!=lastY){
      if(y==1)  menuIndex=(menuIndex+1)%3;   // FIXED: physical DOWN -> move DOWN
      if(y==-1) menuIndex=(menuIndex+2)%3;   // FIXED: physical UP -> move UP
  ...
  void updateMouse(){
    ...
    int y=joy.y();
    if(y!=lastY){
      if(y==-1) ble.resetPointer();            // FIXED: UP = home
      if(y==1)  speedIndex=(speedIndex+1)%3;   // FIXED: DOWN = cycle speed
      lastY=y;
    }
    int x=joy.x();
    if(x!=lastX){ if(x==-1)ble.clickLeft(); if(x==1)ble.clickRight(); lastX=x; }
    float gx=imu.applyDeadZone(ai.filterX(imu.getGyroX()),GYRO_DEAD_ZONE);
    float gy=imu.applyDeadZone(ai.filterY(imu.getGyroY()),GYRO_DEAD_ZONE);
    float s=speeds[speedIndex];
    if(scrollMode){ int8_t sv=(int8_t)constrain((int)(gx*s*2),-127,127); if(sv)ble.addScroll(sv); }
    else { int dx=(int)(gy*s), dy=(int)(gx*s); if(dx||dy)ble.move(dx,dy); }
